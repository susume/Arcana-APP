...
Assert-Contains $ui 'function selectUploadCardSystem(' 'Expected upload flows to let users explicitly constrain tarot versus cartomancy recognition.'
Assert-Contains $ui 'Never choose a tarot card merely to represent a visible number.' 'Expected photo recognition to ignore layout numbers instead of mapping them to tarot cards.'
Assert-Contains $ui 'If a card is unclear, return null instead of guessing a tarot equivalent.' 'Expected playing-card recognition to fail closed instead of inventing tarot substitutes.'
...
Assert-Contains $ui 'Preserve the printed rank and suit' 'Expected playing-card naming guard.'
...
Assert-Contains $readingEngine 'function cleanInfographicHeadline(' 'Expected exported cards to remove duplicated position/card headings.'
Assert-Contains $readingEngine 'const adviceHeight=430' 'Expected infographic guidance to reserve enough height for four wrapped actions.'
Assert-Contains $readingEngine 'function getInfographicTemplate(' 'Expected the export to select a spread-specific infographic template.'
Assert-Contains $readingEngine "spreadLayout==='two-pathways'" 'Expected a paired Two Pathways infographic template.'
Assert-Contains $readingEngine "spreadLayout==='relationship'" 'Expected a paired Relationship infographic template.'
Assert-Contains $readingEngine "spreadLayout==='yearly'" 'Expected a chronological Yearly infographic template.'
Assert-Contains $readingEngine "spreadLayout==='romany'" 'Expected a Past/Present/Future Romany infographic matrix.'
Assert-Contains $readingEngine "id:'focus',variant:'feature'" 'Expected a featured one-card infographic template.'
Assert-Contains $readingEngine "id:'sequence',variant:'compact'" 'Expected a three-card sequence infographic template.'
Assert-NotContains $readingEngine 'Reflection · ${card.action}' 'Expected repetitive per-card focus actions to be omitted from the infographic.'
Assert-Contains $readingEngine 'const labelWidth=Math.min(labelMax' 'Expected position labels to use bounded pills sized for each template.'
Assert-NotContains $readingEngine 'const badgeSize=58' 'Expected the clipped circular position badge to be removed.'
